...
class TelaConfiguracoes:
    def __init__(self):
        ...
        self.botao_volume_menos.posicionar(LARGURA_TELA // 2 - 70, 175)
        self.botao_volume_mais.posicionar(LARGURA_TELA // 2 + 20, 175)

        self.botao_nome = Botao(self.fonte_opcao, "Trocar nome", largura_botao, altura_botao)
        self.botao_nome.posicionar(x_centro, 230)

        self.botao_cor = Botao(self.fonte_opcao, "Cor do titulo", largura_botao, altura_botao)
        self.botao_cor.posicionar(x_centro, 280)

        self.botao_zerar = Botao(self.fonte_opcao, "Zerar recorde", largura_botao, altura_botao)
        self.botao_zerar.posicionar(x_centro, 330)

        self.botao_voltar = Botao(self.fonte_opcao, "Voltar", largura_botao, altura_botao)
        self.botao_voltar.posicionar(x_centro, 410)

    def desenhar(self, tela, volume, cor_titulo):
        self._desenhar_texto(tela, "Configuracoes", self.fonte_titulo, 60, cor_titulo)
        self._desenhar_texto(tela, f"Volume: {round(volume * 100)}%", self.fonte_opcao, 140)
